...
using WinRT;
using IslandWindow = WinUIIslands.Window;

namespace WinUIIslands.TitleBar;

internal sealed partial class CaptionButtonsControl : Control
{
    private const string XamlResourceName = "WinUIIslands.TitleBar.CaptionButtonsControl.xaml";
    ...
    public CaptionButtonsControl()
    {
        ...
        var style = _resources["CaptionButtonsControlStyle"]
            ?? throw new InvalidOperationException("The caption buttons style was not found in the embedded XAML.");

        // XamlReader.Load can leave dynamically created objects behind a generic WinRT wrapper in NativeAOT.
        // JIT recovers the runtime class during a normal cast; AOT needs an explicit QueryInterface for Style.
        Style = style as Style ?? style.As<Style>();
    }
