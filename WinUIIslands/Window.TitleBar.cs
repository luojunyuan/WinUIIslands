...
using WinUIIslandsTitleBar = WinUIIslands.Controls.TitleBar;
...
public unsafe partial class Window
{
    ...
    {
        if (_titleBar is WinUIIslandsTitleBar oldTitleBar && !ReferenceEquals(oldTitleBar, titleBar))
        ...
        _titleBar = titleBar;
        var winUIIslandsTitleBar = titleBar as WinUIIslandsTitleBar;

        if (winUIIslandsTitleBar is not null)
        {
            hitTest ??= winUIIslandsTitleBar.HitTest;
            applyWindowRegion ??= winUIIslandsTitleBar.ApplyTitleBarWindowRegion;
        }

        _titleBarHitTest = hitTest;
        _titleBarWindowRegion = applyWindowRegion;
        winUIIslandsTitleBar?.AttachWindow(this);
    ...
    private void SetActiveCaptionButtons(CaptionButtonsControl? captionButtons)
    {
        ...
        (_titleBar as WinUIIslandsTitleBar)?.RefreshWindowInsets();
    }
    ...
    private bool TryHitTestTitleBar(LPARAM lParam, out LRESULT result)
    {
            ...
            TryGetXamlRootScreenPosition(out var xamlRootScreenX, out var xamlRootScreenY))
        {
            ...
            {
                // Fall back to WinUIIslands's default titlebar hit testing.
            }
        }
    ...
    private void ApplyTitleBarWindowRegion(int topBorderThickness)
    {
            ...
            TryGetXamlRootScreenPosition(out var xamlRootScreenX, out var xamlRootScreenY))
        {
            ...
            {
                // Fall back to WinUIIslands's default rectangular region.
            }
        }
